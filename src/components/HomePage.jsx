...
export default function HomePage({ content, storageUrl }) {
  ...
  return (
    <main className="site-shell">
      ...
      <section className="home-services" id="services"><div className="section-heading-home"><p className="section-tag"><span>What we offer</span></p><h2>Care for every<br /><i>part of life.</i></h2><a className="text-button" href="#about-page">About our approach <span aria-hidden="true">↗</span></a></div><div className="service-grid">{content.services.length ? content.services.map((service, index) => { const image = getPreviewImage(service, 'service-images', content.images['service-images'], index, storageUrl); return <article key={service.id || index}>{image && <img className="service-card-image" src={image} alt="" />}<span className="service-number">0{index + 1}</span><h3>{service.title || service.name}</h3><p>{service.description}</p><span className="service-arrow" aria-hidden="true">↗</span></article> }) : <p>Our services are being updated.</p>}</div></section>
      <section className="home-preview-section home-offers"><div className="preview-heading"><div><p className="section-tag"><span>Packages & promotions</span></p><h2>Good care,<br /><i>made simple.</i></h2></div><a className="text-button" href="#medical-packages">Explore offers <span aria-hidden="true">↗</span></a></div><div className="preview-grid"><div className="preview-column"><h3>Medical packages</h3>{content.packages.slice(0, 2).map((item, index) => <a className="offer-row" href="#medical-packages" key={item.id || index}><span>0{index + 1}</span><strong>{item.title}</strong><em>{item.price ? `$${item.price}` : 'View'} ↗</em></a>)}</div><div className="preview-column"><h3>Promotions</h3>{content.promotions.slice(0, 2).map((item, index) => <a className="offer-row" href="#promotions" key={item.id || index}><span>0{index + 1}</span><strong>{item.title}</strong><em>{item.discount_value ? `${item.discount_value}${item.discount_type === 'Percentage' ? '%' : ''} off` : 'View'} ↗</em></a>)}</div></div></section>
